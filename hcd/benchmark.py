...
from dual_cql_client import DualCQLClient
...
class TableAPIBenchmark:
    ...
    def run_benchmark(self, total_documents: int) -> Dict[str, Any]:
        ...
        """
        logger.info(f"Starting HCD CQL benchmark for {total_documents} documents")
        ...
        try:
            ...
            self.dual_cql_client = DualCQLClient(
            ...
            logger.error(f"HCD CQL benchmark failed: {e}")
            raise
        finally:
            # Cleanup
            if self.dual_cql_client:
                self.dual_cql_client.disconnect()
