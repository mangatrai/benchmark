#!/usr/bin/env python3
"""
Table API Benchmark Entry Point.
Main entry point for the Table API benchmark application.
"""
...
def print_results(results: dict, dry_run: bool = False) -> None:
    ...
    print("=" * 80)
    print(f"TABLE API ASTRA DB BENCHMARK RESULTS")
    print("=" * 80)
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description='Table API AstraDB Benchmark')
    ...
    try:
        ...
        # Run benchmark
        if args.dry_run:
            logger.info(f"Starting Table API dry run for {total_records} records")
            results = benchmark.run_dry_run(total_records)
        else:
            logger.info(f"Starting Table API benchmark for {total_records} records")
