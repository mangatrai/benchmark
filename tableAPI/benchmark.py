...
class TableAPIBenchmark:
        ...
        try:
            while documents_generated < total_documents and not self.stop_event.is_set():
                ...
                for document in batch:
                    ...
                    json_payload = json.dumps(document, default=str)
                    json_bytes = len(json_payload.encode('utf-8'))
                    batch_bytes += json_bytes
                    
                    # Calculate actual data sizes
                    chunk = document.get('chunk', '')
                    vector = document.get('vector_column', [])
                    ...
                    # Metadata calculations (approximate)
                    metadata_bytes = json_bytes - chunk_bytes - (vector_bytes if isinstance(vector, list) else 0)
        ...
        try:
            while not self.stop_event.is_set():
                try:
                    ...
                    batch = self.document_queue.get(timeout=1.0)
                    
                    # Insert batch into AstraDB
                    self.table_client.insert_batch(batch)
    ...
    def run_benchmark(self, total_documents: int) -> Dict[str, Any]:
        ...
        try:
            ...
            self.table_client = TableAPIClient(
                token=self.config.astra_token,
                endpoint=self.config.astra_endpoint,
                namespace=self.config.astra_namespace,
                table=self.config.astra_table,
                max_retries=self.config.max_retries,
                retry_delay=self.config.retry_delay
            )
            
            # Connect to AstraDB
            self.table_client.connect()
